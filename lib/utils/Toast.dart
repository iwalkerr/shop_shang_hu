...
class Toast {
	...
	static void toast(
		...
		//2秒后 到底消失不消失
		if (_overlayEntry != null && DateTime.now().difference(_startedTime).inMilliseconds >= _showTime) {
			_showing = false;
			_overlayEntry?.markNeedsBuild();
...
//  设置toast位置
	static _calToastPosition(context) {
		...
		if(_toastPosition == 'top'){
			...
			backResult = MediaQuery.of(context).size.height * 7 / 8;
		}else{
			backResult = MediaQuery.of(context).size.height * 3 / 4;
		}

		return backResult + _topAdd;
	}
}